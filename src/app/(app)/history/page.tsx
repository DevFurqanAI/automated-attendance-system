...
import { HistoryViewTabs } from './HistoryViewTabs';
...
export default async function HistoryPage() {
  ...
  const [
    ...
    supabase
      .from('attendance')
      .select(
        '*, branches:branch_id ( id, name ), employees:employee_id ( id, full_name, email )',
      )
  ...
  const daysByDate: Record<string, { status: DayStatus; items: DayItem[] }> =
    {};
  for (const [date, items] of dayItems) {
    const status = dominantStatus(items);
    if (status) daysByDate[date] = { status, items };
  }
  const latestDate = rows[0]
    ? (rows[0].check_in_time ?? rows[0].submitted_at).slice(0, 7)
    : new Date().toISOString().slice(0, 7);

  const approvedHours = rows
    .filter((r) => r.status === 'approved')
    .reduce(
      (sum, r) => sum + (hoursWorked(r.check_in_time, r.check_out_time) ?? 0),
      0,
    );
                      ...
                      row.check_in_time && !showClaim
                        ? lateMinutes(
                            ...
                            resolveExpectedStartTime(
                              ...
                              row.branch_id
                                ? (branchStartById.get(row.branch_id) ?? null)
                                : null,
                            ),
                          )
                        : null;
